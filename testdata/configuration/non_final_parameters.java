...
public final class WidgetMatcher extends CompositePropertyMatcher<Widget> {
    ...
    private WidgetMatcher(String matchedObjectDescription, Widget template) {
    ...
    public static WidgetMatcher aWidgetLike(Widget template) {
        return new WidgetMatcher(MATCHED_OBJECT_DESCRIPTION, template);
    }

    public WidgetMatcher hasProp(String prop) {
        return hasProp(equalTo(prop));
    }

    public WidgetMatcher hasProp(Matcher<? super String> propMatcher) {
        this.propMatcher.setMatcher(propMatcher);
        return this;
    }

    @Override
    protected void matchesSafely(Widget item, MatchAccumulator matchAccumulator) {
