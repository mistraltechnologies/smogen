...
public final class WidgetMatcher extends CompositePropertyMatcher<Widget> {
    ...
    private final PropertyMatcher<Integer> superPropMatcher = new ReflectingPropertyMatcher<Integer>("superProp", this);

    private WidgetMatcher(final String matchedObjectDescription, final Widget template) {
        super(matchedObjectDescription);
        if (template != null) {
            hasProp(template.getProp());
            hasSuperProp(template.getSuperProp());
        }
    }
    ...
    public WidgetMatcher hasSuperProp(final int superProp) {
        return hasSuperProp(equalTo(superProp));
    }

    public WidgetMatcher hasSuperProp(final Matcher<? super Integer> superPropMatcher) {
        this.superPropMatcher.setMatcher(superPropMatcher);
        return this;
    }
